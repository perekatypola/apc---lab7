...
void Server(char* path)
{
	...
	SetCommMask(handler, EV_RXCHAR);										// Определяет пакет событий, которые будут отсоеживаться для устройства. В данном случае - получен байт
	...
	CommTimeOuts.ReadTotalTimeoutMultiplier = 0xFFFFFFFF;							// Множитель, используемый, чтобы вычислить полный период времени простоя для операций чтения.
	CommTimeOuts.ReadTotalTimeoutConstant = 10;						// Константа, используемая, чтобы вычислить полный (максимальный) период времени простоя для операций чтения.
	...
	CommTimeOuts.WriteTotalTimeoutConstant = 0;						// Константа, используемая, чтобы вычислить полный период времени простоя для операций записи.
	...
	// Структура, характеризующая основные параметры последовательного порта. 
	DCB ComDCM;																
	...
	ComDCM.Parity = NOPARITY;												// Определяет выбор схемы контроля четности (Бит четности отсутствует).
	...
	if (!SetCommState(handler, &ComDCM))									//конфигурирует коммуникационное устройство согласно определениям в управляющем устройством блок
	{
		CloseHandle(handler);
		handler = INVALID_HANDLE_VALUE;
		return;
